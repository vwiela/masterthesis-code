...
@everywhere begin
    ...
    particles_set = [100,200,300,400,500]
...
@everywhere workers() begin
    ...
    problem = pypesto.Problem(
        objective,
        x_names=["gamma", "kappa", "beta", "tevent"],
        lb=[10,1,0.01,130], # parameter bounds
        ub=[25,9,1,360], # NB for sampling it is usually better if you remap parameters to (-∞, ∞)
...
chs = MCMCChains.Chains(chains, [:gamma, :kappa, :beta, :tevent, :lp])
complete_chain = set_section(chs, Dict(:parameters => [:gamma, :kappa, :beta, :tevent], :internals => [:lp]))
...
# store results
h5open("./output/synthetic/adapt_synthetic_no_prior_"*string(nworkers())*"chs_"*string(niter)*"it_"*string(nparticles)*"p.h5", "w") do f
  write(f, complete_chain)
end


open("./output/synthetic/time_adapt_synthetic_no_prior_"*string(nworkers())*"chs_"*string(niter)*"it_"*string(nparticles)*"p.txt", "w") do file
